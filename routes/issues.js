...
const {validateArticle, validateComment} = require('../controllers/validation'); // validation schema

const prefix = '/api/v1/articles'; // URI endpoint to view the articles stores on db
const router = Router({prefix: prefix});

// issue routes
router.get('/', getAll);
router.post('/', auth, bodyParser(), validateArticle, createArticle); // posting data
router.get('/:id([0-9]{1,})', getById);
router.put('/:id([0-9]{1,})', auth, bodyParser(), validateArticle, updateArticle);
router.del('/:id([0-9]{1,})', auth, deleteArticle); // validateArticle = middleware
